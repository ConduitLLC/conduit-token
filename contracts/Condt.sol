...
contract ConduitToken is MintableToken, LimitedTransferToken {

    string public constant name = "ConduitCoin";
    ...
    function ConduitToken(uint256 _endTimeICO, address _bountyWallet) {
...
contract ConduitTokenCrowdsale is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public constant CAP = 40000 ether;
    uint8 public constant decimals = 18;
    uint256 public constant DECIMALSFACTOR  = 10**uint256(decimals);
    uint256 public constant TOKEN_CAP = 1000000000 * DECIMALSFACTOR;
    ...
    function ConduitTokenCrowdsale(
        uint256[11] localtimings,
        uint8[10] _bonuses,
        address[3] _wallets,
        ...
        uint64 advisersReleaseTime
            ...
            require(localtimings[0] >= now);

            for(uint i = 1; i < timings.length; i++) {
              require(localtimings[i] >= localtimings[i-1]);
            }
            ...
            token = new ConduitToken(endTime, bountyWallet);
